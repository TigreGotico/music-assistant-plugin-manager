...
If the domain is known, `find_spec` locates the real spec via `importlib.util.find_spec(real_module_name)` and returns a new `ModuleSpec` pointing at `_AliasLoader`. `_AliasLoader.exec_module` loads the real module (or reuses it from `sys.modules`) and copies all attributes into the alias module object, then registers the real module under its own name in `sys.modules` as well.
...
A `.pth` file with executable code runs during site-package initialisation, very early in the Python startup sequence — before the user's process code. This makes it impossible to guarantee ordering relative to other site-packages hooks and creates hard-to-debug import side effects. The wrapper launcher approach keeps the patch explicit, traceable, and testable.
